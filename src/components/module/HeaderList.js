...
function HeaderList() {
  ...
  return (
    <div className="fixed top-6 right-0 left-0 z-50 mx-auto hidden h-24 w-9/10 items-center justify-between rounded-3xl bg-black/50 px-6 font-Dana text-xl tracking-tightest text-gray-300 backdrop-blur-[6px] lg:flex lg:text-base">
              ...
              {item.title === "فروشگاه" && (
                <ul className="absolute top-20 flex max-h-0 translate-y-2 flex-col space-y-1.5 overflow-hidden rounded-2xl border-orange-300 bg-white tracking-normal text-zinc-700 opacity-0 shadow-app transition-all duration-500 ease-in-out group-hover:max-h-56 group-hover:translate-y-0 group-hover:border-t-[3px] group-hover:p-6 group-hover:opacity-100 dark:bg-zinc-700 dark:text-white">
                  {coffeeList.map((item) => (
                    <li
                      key={item.id}
                      className="cursor-pointer transition-colors duration-200 hover:text-orange-300"
                    >
                      {" "}
                      {item.title}{" "}
                    </li>
      ...
      <div className="flex items-center">
        <div className="flex items-center">
          <div className="group flex items-center">
            <span
              className={`${transitionClass} relative inline-block h-6 w-6 pr-3 pl-2.5 hover:text-orange-300 lg:h-18 lg:w-11.75 xl:w-14`}
            >
              {" "}
            ...
            <div
              ...
              className="absolute top-20 left-66 max-h-0 overflow-hidden rounded-2xl bg-white opacity-0 transition-all duration-500 border-top-3 group-hover:max-h-83 group-hover:opacity-100 dark:bg-zinc-700"
            >
              <div className="mt-6 flex items-center justify-between px-6">
                <span> 1مورد </span>
                <span className="flex cursor-pointer items-center hover:text-orange-300">
                  {" "}
                  مشاهده سبد خرید
              ...
              <p className="mx-auto mt-5 h-px w-90 bg-gray-300"></p>
          ...
          {/* </span> */}
          <span
            className={`${transitionClass} hover:text-orange-300 lg:size-6 xl:size-8.5`}
        ...
        <div className="mx-8 h-14 w-px bg-[#FFFFFF33]"></div>
